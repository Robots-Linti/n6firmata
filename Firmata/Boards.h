...
// Original code written for Teensy 2.0 (commented block)
// changes made to use with DuinoBot
#elif defined(__AVR_ATmega32U4__)

#define TOTAL_ANALOG_PINS       6 //ok
#define TOTAL_PINS              20 //ok
#define VERSION_BLINK_PIN       13 //ok
#define IS_PIN_DIGITAL(p)       ((p) >= 2 && (p) <= 19 && (p)!=3 && (p)!=4 && (p)!=8)
#define IS_PIN_ANALOG(p)        ((p) >= 14 && (p) <= 19)
#define IS_PIN_PWM(p)           ((p)==5 || (p)==6 || (p)==9)
#define IS_PIN_SERVO(p)         (0)
#define IS_PIN_I2C(p)           (0)
#define PIN_TO_DIGITAL(p)       (p)
#define PIN_TO_ANALOG(p)        (19-(p))
#define PIN_TO_PWM(p)           PIN_TO_DIGITAL(p)
#define PIN_TO_SERVO(p)         (p)
